begin;
create schema if not exists "project_agent_eta_v2";
-- Agent ETA cloud data is private by default. Only pre-aggregated snapshots are "project_agent_eta_v2".
...
create or replace function "project_agent_eta_v2".agent_eta_set_updated_at()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
create table "project_agent_eta_v2".private_runs (
...
create index private_runs_user_created_idx
  on "project_agent_eta_v2".private_runs (user_id, client_created_at desc);
create index private_runs_user_calibration_idx
  on "project_agent_eta_v2".private_runs (user_id, provider, task_class, model, effort, speed)
  where outcome = 'success';

create trigger private_runs_set_updated_at
before update on "project_agent_eta_v2".private_runs
for each row execute function "project_agent_eta_v2".agent_eta_set_updated_at();

create table "project_agent_eta_v2".user_sync_settings (
...
before update on "project_agent_eta_v2".user_sync_settings
for each row execute function "project_agent_eta_v2".agent_eta_set_updated_at();

create table "project_agent_eta_v2".benchmark_contributions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  private_run_id uuid not null references "project_agent_eta_v2".private_runs(id) on delete cascade,
  provider text not null,
  model text not null,
  effort text not null,
  speed text not null,
...
create index benchmark_contributions_aggregate_idx
  on "project_agent_eta_v2".benchmark_contributions (run_created_at, provider, model, task_class);
create index benchmark_contributions_user_idx
  on "project_agent_eta_v2".benchmark_contributions (user_id, contributed_at desc);

create table "project_agent_eta_v2".public_metric_snapshots (
  id uuid primary key default gen_random_uuid(),
...
create index public_metric_snapshots_dimension_idx
  on "project_agent_eta_v2".public_metric_snapshots (dimension_kind, provider, model, task_class);
create index public_metric_snapshots_generated_idx
  on "project_agent_eta_v2".public_metric_snapshots (generated_at desc);

alter table "project_agent_eta_v2".private_runs enable row level security;
alter table "project_agent_eta_v2".user_sync_settings enable row level security;
alter table "project_agent_eta_v2".benchmark_contributions enable row level security;
alter table "project_agent_eta_v2".public_metric_snapshots enable row level security;

create policy "Users read their private runs"
on "project_agent_eta_v2".private_runs for select to authenticated
using ((select auth.uid()) = user_id);

create policy "Users create their private runs"
on "project_agent_eta_v2".private_runs for insert to authenticated
with check ((select auth.uid()) = user_id);

create policy "Users update their private runs"
on "project_agent_eta_v2".private_runs for update to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

create policy "Users delete their private runs"
on "project_agent_eta_v2".private_runs for delete to authenticated
using ((select auth.uid()) = user_id);

create policy "Users read their sync settings"
on "project_agent_eta_v2".user_sync_settings for select to authenticated
using ((select auth.uid()) = user_id);

create policy "Users create their sync settings"
on "project_agent_eta_v2".user_sync_settings for insert to authenticated
with check ((select auth.uid()) = user_id);

create policy "Users update their sync settings"
on "project_agent_eta_v2".user_sync_settings for update to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

create policy "Users delete their sync settings"
on "project_agent_eta_v2".user_sync_settings for delete to authenticated
using ((select auth.uid()) = user_id);

create policy "Users read their contributions"
on "project_agent_eta_v2".benchmark_contributions for select to authenticated
using ((select auth.uid()) = user_id);

create policy "Users retract their contributions"
on "project_agent_eta_v2".benchmark_contributions for delete to authenticated
using ((select auth.uid()) = user_id);

create policy "Anyone reads safe metric snapshots"
on "project_agent_eta_v2".public_metric_snapshots for select to anon, authenticated
using (true);

create or replace function "project_agent_eta_v2".contribute_private_run(
...
begin
  ...
  from "project_agent_eta_v2".user_sync_settings
  ...
  insert into "project_agent_eta_v2".benchmark_contributions (
    user_id,
    private_run_id,
    provider,
    model,
...
create or replace function "project_agent_eta_v2".agent_eta_remove_contributions_on_opt_out()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if tg_op = 'DELETE' then
    delete from "project_agent_eta_v2".benchmark_contributions where user_id = old.user_id;
    return old;
  end if;
...
after update of benchmark_contribution_enabled or delete on "project_agent_eta_v2".user_sync_settings
for each row execute function "project_agent_eta_v2".agent_eta_remove_contributions_on_opt_out();

create or replace function "project_agent_eta_v2".refresh_agent_eta_public_metrics(
...
begin
  ...
  delete from "project_agent_eta_v2".public_metric_snapshots;
  ...
  with eligible as (
    select
      ...
      forecast_p80_minutes,
      actual_minutes
    from "project_agent_eta_v2".benchmark_contributions
  ...
  insert into "project_agent_eta_v2".public_metric_snapshots (
    snapshot_key,
    dimension_kind,
    provider,
    model,
...
revoke all on table "project_agent_eta_v2".private_runs from public, anon, authenticated;
revoke all on table "project_agent_eta_v2".user_sync_settings from public, anon, authenticated;
revoke all on table "project_agent_eta_v2".benchmark_contributions from public, anon, authenticated;
revoke all on table "project_agent_eta_v2".public_metric_snapshots from public, anon, authenticated;

grant select, insert, update, delete on table "project_agent_eta_v2".private_runs to authenticated;
grant select, insert, update, delete on table "project_agent_eta_v2".user_sync_settings to authenticated;
grant select, delete on table "project_agent_eta_v2".benchmark_contributions to authenticated;
grant select on table "project_agent_eta_v2".public_metric_snapshots to anon, authenticated;
grant all on table "project_agent_eta_v2".private_runs to service_role;
grant all on table "project_agent_eta_v2".user_sync_settings to service_role;
grant all on table "project_agent_eta_v2".benchmark_contributions to service_role;
grant all on table "project_agent_eta_v2".public_metric_snapshots to service_role;

revoke all on function "project_agent_eta_v2".agent_eta_set_updated_at() from public, anon, authenticated;
revoke all on function "project_agent_eta_v2".contribute_private_run(uuid, text) from public, anon;
revoke all on function "project_agent_eta_v2".agent_eta_remove_contributions_on_opt_out() from public, anon, authenticated;
revoke all on function "project_agent_eta_v2".refresh_agent_eta_public_metrics(timestamptz, timestamptz) from public, anon, authenticated;

grant execute on function "project_agent_eta_v2".contribute_private_run(uuid, text) to authenticated, service_role;
grant execute on function "project_agent_eta_v2".refresh_agent_eta_public_metrics(timestamptz, timestamptz) to service_role;

comment on table "project_agent_eta_v2".private_runs is
  'User-owned derived Agent ETA run data. Raw prompts, repository names, paths, and code are prohibited by schema.';
comment on table "project_agent_eta_v2".benchmark_contributions is
  'Consent-gated copies of safe run fields. Raw rows are never publicly readable.';
comment on table "project_agent_eta_v2".public_metric_snapshots is
  'Public, service-generated aggregate metrics. Each cohort has at least 25 runs from 20 contributors.';
comment on function "project_agent_eta_v2".refresh_agent_eta_public_metrics(timestamptz, timestamptz) is
...
create or replace function "project_agent_eta_v2".agent_eta_normalize_benchmark_consent()
returns trigger
language plpgsql
security definer
set search_path = ''
...
on "project_agent_eta_v2".user_sync_settings;
...
on "project_agent_eta_v2".user_sync_settings
for each row execute function "project_agent_eta_v2".agent_eta_normalize_benchmark_consent();
...
revoke all on function "project_agent_eta_v2".agent_eta_normalize_benchmark_consent() from public, anon, authenticated;
grant execute on function "project_agent_eta_v2".agent_eta_normalize_benchmark_consent() to service_role;

revoke all on function "project_agent_eta_v2".contribute_private_run(uuid, text) from public, anon;
grant execute on function "project_agent_eta_v2".contribute_private_run(uuid, text) to authenticated, service_role;

comment on function "project_agent_eta_v2".agent_eta_normalize_benchmark_consent() is
  'Server-normalizes future-only benchmark consent timestamps and blocks client backdating.';

comment on function "project_agent_eta_v2".contribute_private_run(uuid, text) is
...
create or replace function "project_agent_eta_v2".agent_eta_preserve_private_run_origin()
returns trigger
language plpgsql
security definer
set search_path = ''
...
drop trigger if exists private_runs_preserve_origin on "project_agent_eta_v2".private_runs;

create trigger private_runs_preserve_origin
before update on "project_agent_eta_v2".private_runs
for each row execute function "project_agent_eta_v2".agent_eta_preserve_private_run_origin();

revoke all on function "project_agent_eta_v2".agent_eta_preserve_private_run_origin() from public, anon, authenticated;
grant execute on function "project_agent_eta_v2".agent_eta_preserve_private_run_origin() to service_role;

comment on function "project_agent_eta_v2".agent_eta_preserve_private_run_origin() is
