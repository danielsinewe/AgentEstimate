...
- **About** is the modeled midpoint: about half of comparable runs should finish sooner.
- **Allow up to** is the practical planning bound: about four in five should finish sooner once observed history supports that coverage.
...
| van der Bles and colleagues' [experiments on communicating uncertainty](https://doi.org/10.1073/pnas.1913678117) recommend a numeric range with a point estimate and found that uncertainty need not reduce trust in the source. | Hiding uncertainty is unnecessary; contextualizing it is the important part. | Keep both an expected time and a planning bound rather than collapsing to one confident-looking number. |
| The ONS [plain-language standard](https://service-manual.ons.gov.uk/content/writing-for-users/plain-language) and GOV.UK [UI writing guidance](https://www.gov.uk/service-manual/design/writing-for-user-interfaces) advise avoiding unexplained technical terms and minimizing cognitive load, including for specialists. | Expert audiences also benefit from direct, scannable labels. | Use “About” and “Allow up to” in the product; reserve P50/P80 for technical drill-downs. |
